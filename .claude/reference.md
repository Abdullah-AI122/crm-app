...
Every blank is an INLINE token sized to its content, never a full-width field: dashed + muted when empty ("something goes here"), solid accent when filled ("a word in the sentence"). Blanks stay native select/input — keyboard, mobile pickers and screen-reader labelling all come free, and a hand-rolled popup has to re-earn each.
...
Naming comes LAST and is optional, defaulted from the sentence. Being asked to name a thing before describing it is the wrong order.
